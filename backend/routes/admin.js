...
router.get('/system-status', adminMiddleware, async (req, res) => {
  try {
    ...
    // Offene Projekte (nicht abgeschlossen, status != 'geplant' und != 'abgeschlossen')
    const offeneProjekte = await pool.query(`
      SELECT pa.id, pa.status, pa.mitarbeiter_name, pa.kundenname, pa.adresse,
             pa.fahrt_start, pa.auftragsnummer,
             ag.name AS auftraggeber_name
      FROM projekt_auftraege pa
      LEFT JOIN auftraggeber ag ON ag.id = pa.auftraggeber_id
      WHERE pa.status NOT IN ('abgeschlossen','geplant')
      ORDER BY pa.fahrt_start DESC NULLS LAST
      LIMIT 100
    `);
    res.json({ offene_einsaetze: offene.rows, eingestempelt: eingestempelt.rows, offene_projekte: offeneProjekte.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── Force-Close Projekt ─────────────────────────────────────────────────────
router.post('/projekt/:id/force-close', adminMiddleware, async (req, res) => {
  try {
    const r = await pool.query(
      `UPDATE projekt_auftraege
       SET status='abgeschlossen', abgeschlossen_am=COALESCE(abgeschlossen_am,NOW()),
    ...
    if (!r.rows.length) return res.status(404).json({ error: 'Projekt nicht gefunden' });
    await logEvent({
      level: 'warn', aktion: 'force-close-projekt',
      ausgeloest_von: req.user.name,
      details: { projekt_id: req.params.id, mitarbeiter: r.rows[0].mitarbeiter_name, kunde: r.rows[0].kundenname },
      ip: req.ip
    });
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
