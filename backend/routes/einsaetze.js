...
router.post('/start', authMiddleware, async (req, res) => {
  ...
  try {
    // Doppel-Start verhindern: existiert bereits ein offener Einsatz für diesen MA+Anlage+Datum?
    const existing = await pool.query(
      `SELECT * FROM einsaetze WHERE mitarbeiter_id=$1 AND anlage_id=$2 AND datum=CURRENT_DATE
       AND (sparte=$3 OR sparte IS NULL) AND end_zeit IS NULL LIMIT 1`,
      [req.user.id, anlage_id, sparte || 'winterdienst']
    );
